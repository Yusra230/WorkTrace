...
Return exactly one non-empty evidence mapping for each competency. Copy event_id values exactly as they appear in the supplied events; never invent an ID, substitute an event type, or cite an event by its sequence. Each mapping must contain the learner-owned evidence required by the supplied attribution categories; AI events may be cited only as context alongside that learner evidence. In particular, independent_judgment must cite both a learner decision event and a learner independent_explanation (follow-up answer) event. In every explanation, state the event's actor/category accurately and do not attribute AI-provided information to the learner. Return only the requested structured data.`;
