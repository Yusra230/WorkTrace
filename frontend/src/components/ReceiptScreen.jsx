...
export default function ReceiptScreen({ receipt }) {
  const evidenceByDimension = new Map((receipt?.evidence || []).map((item) => [item.dimension, item]));

  return (
    <section className="mx-auto max-w-6xl space-y-6">
      <header className="rounded-2xl border border-cyan-300/25 bg-gradient-to-br from-cyan-300/10 via-slate-900/70 to-violet-300/10 p-6 sm:p-8">
        ...
        <p className="mt-5 text-sm font-semibold uppercase tracking-[0.15em] text-cyan-300">Competency Receipt</p>
        <h1 className="mt-2 text-3xl font-semibold tracking-tight text-white sm:text-4xl">Evidence of how you work with AI.</h1>
        <p className="mt-4 max-w-3xl leading-7 text-slate-300">This assessment is grounded in your recorded investigation, decision, verification, and independent explanation.</p>
      </header>
      ...
      <section aria-labelledby="evidence-heading" className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6">
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          {dimensions.map(([key, label]) => {
            ...
            return <article key={key} className="rounded-xl border border-slate-800 bg-slate-950/30 p-4"><h3 className="font-medium text-slate-100">{label}</h3>{evidence ? <><p className="mt-2 text-sm leading-6 text-slate-300">{evidence.explanation}</p><div className="mt-4 flex flex-wrap gap-2">{evidence.event_ids.map((id, index) => <span key={id} className="rounded-md border border-slate-700 bg-slate-900 px-2 py-1 font-mono text-xs text-slate-300">Event {evidence.event_sequences?.[index] ? `#${evidence.event_sequences[index]} · ` : ''}{id}</span>)}</div></> : <p className="mt-2 text-sm text-slate-500">No mapped evidence returned for this dimension.</p>}</article>;
          })}
        </div>
      </section>
